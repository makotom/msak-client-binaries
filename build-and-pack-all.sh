...
SRC_REPO=${SRC_REPO:-"https://github.com/m-lab/msak.git"}
...
build_gc() {
    ...
    OUTPUT="$(pwd)/dist/${goos}/${goarch}/msak-client"
    if [[ "${goos}" == "windows" ]]; then
        OUTPUT="$(pwd)/dist/${goos}/${goarch}/msak-client.exe"
    fi

    pushd src/cmd/msak-client
...
package() {
    ...
    cp --no-preserve=all src/LICENSE -t "dist/${goos}/${goarch}"

    pushd "dist/${goos}/${goarch}"
    if [[ "${goos}" == "linux" ]] || [[ "${goos}" == "android" ]]; then
        tar -czf "../../msak-client-${BUILD_NAME}-${goos}-${goarch}.tar.gz" -- *
    else
        zip -r "../../msak-client-${BUILD_NAME}-${goos}-${goarch}.zip" -- *
    fi
    popd
}
